...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	Microsoft::WRL::ComPtr<ID3D12Resource> transformationMatrixResourceSprite = CreateBufferResource(device, sizeof(TransformationMatrix) * kNumInstance);
	//データを書き込む
	...
	for (uint32_t i = 0; i < kNumInstance; i++) {
		transformationMatrixDataSprite[i].WVP = MakeIdentity4x4();
		transformationMatrixDataSprite[i].World = MakeIdentity4x4();
	}
	//トランスフォーム用の変数
	Transform transformsSprite[kNumInstance];
	for (uint32_t i = 0; i < kNumInstance; i++) {
		transformsSprite[i].scale = { 1.0f,1.0f,1.0f };
		transformsSprite[i].rotate = { 0.0f,0.0f,0.0f };
		transformsSprite[i].translate = { i * 0.1f,i * 0.1f ,i * 0.1f };
	}
	//uvTransform用の変数
	...
	//SRVの生成
	device->CreateShaderResourceView(transformationMatrixResourceSprite.Get(), &instancingSrvDesc, instancingSrvHandleCPU);
	...
	while (msg.message != WM_QUIT) {
		...
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			/////レンダリングパイプライン/////
			for (uint32_t i = 0; i < kNumInstance; i++) {
				Matrix4x4 worldMatrix = MakeAffineMatrix(transformsSprite[i].scale, transformsSprite[i].rotate, transformsSprite[i].translate);
				Matrix4x4 cameraMatrix = MakeAffineMatrix(cameraTransform.scale, cameraTransform.rotate, cameraTransform.translate);
				Matrix4x4 viewMatrix = Inverse(cameraMatrix);
				Matrix4x4 projectionMatrix = MakePerspectiveFovMatrix(0.45f, float(kClientWidth) / float(kClientHeight), 0.1f, 100.0f);
				Matrix4x4 worldViewProjectionMatrix = Multiply(worldMatrix, Multiply(viewMatrix, projectionMatrix));
			...
			//TransformationMatrixStructureBufferの場所を指定
