...
void ParticleManager::Update(const BaseCamera* camera) {
	//ビルボード行列の計算
	...
	for (std::unordered_map<std::string, Struct::ParticleGroup>::iterator particleGroupIterator = particleGroups_.begin(); particleGroupIterator != particleGroups_.end();) {
		...
		for (std::list<Struct::Particle>::iterator particleIterator = particleGroup->particles.begin(); particleIterator != particleGroup->particles.end();) {
			...
			if (MyMath::IsCollision(accelerationField.area, (*particleIterator).transform.translate)) {
				(*particleIterator).velocity = (*particleIterator).velocity + (kDeltaTime * accelerationField.acceleration);
			}
			//移動処理
			(*particleIterator).transform.translate = (*particleIterator).transform.translate + (kDeltaTime * (*particleIterator).velocity);
			//α値設定
...
void ParticleManager::Draw(const BaseCamera* camera) {
	// ルートシグネチャの設定
...
void ParticleManager::CreateParticleGroup(const std::string& name, const std::string& modelFilePath) {
	...
	//インスタンシング用リソースの生成
	particleGroup.instancingResource = DirectXCommon::GetInstance()->CreateBufferResource(sizeof(Struct::ParticleForGPU) * kNumMaxInstance_);
	...
	DirectXCommon::GetInstance()->GetDevice()->CreateShaderResourceView(particleGroup.instancingResource.Get(), &srvDesc, srvManager_->GetCPUDescriptorHandle(particleGroup.instancingSrvIndex));
	//新たに作ったパーティクルグループを登録
