...
ShapeCommon* ShapeCommon::GetInstance()
{
	if (instance == nullptr) {
		instance = new ShapeCommon;
	}
	return instance;
}

void ShapeCommon::Initialize()
{
	//グラフィックスパイプラインの生成
	GenerateGraphicsPipeline();
}

void ShapeCommon::Finalize()
{
	delete instance;
	instance = nullptr;
}

void ShapeCommon::SettingCommonDrawing()
{
	//ルートシグネチャをセットするコマンド
...
void ShapeCommon::GenerateGraphicsPipeline()
{
	HRESULT hr;
	...
	vertexShaderBlob = DirectXCommon::GetInstance()->CompileShader(L"Resources/shaders/Shape.VS.hlsl",
		L"vs_6_0");
	assert(vertexShaderBlob != nullptr);

	Microsoft::WRL::ComPtr<IDxcBlob> pixelShaderBlob = DirectXCommon::GetInstance()->CompileShader(L"Resources/shaders/Shape.PS.hlsl",
		L"ps_6_0");
